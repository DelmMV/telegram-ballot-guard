...
const compactCreatePollScene = () => {
  ...
  if (checkData.importedMentions && checkData.importedMentions.length > 0) {
    ...
    if (checkData.originalPollTitle) {
      ctx.wizard.state.pollData.originalPollTitle = checkData.originalPollTitle;
    }
    }
    ...
    if (ctx.message) {
      try {
        ...
        logger.warn('Could not delete original /createpoll command message:', error);
      }
    }
    ...
    if (ctx.scene.state?.checkData) {
      ...
      if (ctx.scene.state.checkData.interfaceMessageId) {
        ctx.wizard.state.pollData.interfaceMessageId = ctx.scene.state.checkData.interfaceMessageId;
      }
    
      // Store message ID if available
      if (ctx.scene.state.checkData.messageId) {
        ctx.wizard.state.pollData.originalPollMessageId = ctx.scene.state.checkData.messageId;
      }
    } else if (ctx.session?.checkVoters) {
      // New format through session
      if (ctx.session.checkVoters.mentionsList?.length > 0) {
        ctx.wizard.state.pollData.importedMentions = ctx.session.checkVoters.mentionsList;
        ctx.wizard.state.pollData.selectedOption = ctx.session.checkVoters.selectedOption;
        ctx.wizard.state.pollData.title = t('scenes.voters.followUpTitle', { 
          option: ctx.session.checkVoters.selectedOption 
        });
        
        // Save original poll title if available
        if (ctx.session.checkVoters.selectedPollTitle || ctx.session.checkVoters.pollTitle) {
          ctx.wizard.state.pollData.originalPollTitle = ctx.session.checkVoters.selectedPollTitle || ctx.session.checkVoters.pollTitle;
        }
      ...
      if (ctx.session.checkVoters.interfaceMessageId) {
        ctx.wizard.state.pollData.interfaceMessageId = ctx.session.checkVoters.interfaceMessageId;
      }
    
      // Store message ID if available
      if (ctx.session.checkVoters.messageId) {
        ctx.wizard.state.pollData.originalPollMessageId = ctx.session.checkVoters.messageId;
      }
  } else {
  ...
  const getMainMenuText = (ctx) => {
    ...
    if (fromPrivate) {
      text.push(`🔄 ${t('scenes.poll.creatingInGroup', { chatId }) || `Creating poll in group (${chatId})`}`);
    }
    ...
    if (fromCheckVoters && importedMentions && importedMentions.length > 0) {
      ...
      text.push(`${t('scenes.poll.usingImportedMentions', { count, option: selectedOption })}`);
    }
    ...
    // Title section
    text.push(`\n*${t('scenes.poll.titleSection')}*: ${title || t('scenes.poll.notSet')}`);
    ...
    // Mentions section
    const mentionCount = (mentions?.length || 0) + (importedMentions?.length || 0);
    if (mentionCount > 0) {
      ...
      if (selectedOption) {
        text.push(`(${t('scenes.voters.votedFor', { option: selectedOption }) || `Voted for "${selectedOption}"`})`);
      }
    }
  ...
  const getMainMenuButtons = (ctx) => {
    ...
    // Title edit button
    buttons.push([Markup.button.callback(t('scenes.poll.editTitleButton'), 'edit_title')]);
    ...
    // Add option button
    if (options.length < 10) { // Telegram limit is 10 options
      buttons.push([Markup.button.callback(t('scenes.poll.addOptionButton'), 'add_option')]);
    }
    ...
    if (ctx.wizard.state.pollData.title && options.length >= 2) {
      actionButtons.push(Markup.button.callback(t('scenes.poll.createButton'), 'create_poll'));
    }
    
    // Cancel button
    actionButtons.push(Markup.button.callback(t('scenes.poll.cancelButton'), 'cancel_creation'));
  ...
  const createPoll = async (ctx) => {
    const { t } = ctx.i18n;
    const { title, options, mentions, importedMentions, chatId, fromPrivate } = ctx.wizard.state.pollData;
    ...
    try {
      ...
      if (mentionsString) {
        ...
        const messageKey = ctx.wizard.state.pollData.fromCheckVoters === true 
      ...
      // Try to delete the main interface message
      try {
        await ctx.deleteMessage(ctx.wizard.state.pollData.messageId);
      } catch (error) {
        logger.warn('Could not delete interface message:', error);
      }
      
      // We've already deleted the command message at initialization,
      // so we don't need to try again here
      
      // Try to delete the interface message from checkVoters if it exists
      try {
        if (ctx.wizard.state.pollData.interfaceMessageId) {
          await ctx.deleteMessage(ctx.wizard.state.pollData.interfaceMessageId);
        }
      } catch (error) {
        logger.warn('Could not delete checkVoters interface message:', error);
      }
