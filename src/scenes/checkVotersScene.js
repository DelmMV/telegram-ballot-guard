...
const checkVotersScene = () => {
  ...
  const handleCallback = async (ctx) => {
    ...
    try {
      ...
      if (!ctx.callbackQuery) {
        ...
        if (ctx.message && ctx.message.text && ctx.message.text.startsWith('/checkvoters')) {
    ...
    if (action === 'poll_info' || action === 'polls_page_info' || action === 'polls_page_none') {
      await ctx.answerCbQuery();
      return;
    }
    ...
    if (action === 'polls_page_next') {
      try {
        ...
        const totalItems = ctx.session.checkVoters.availablePolls ? ctx.session.checkVoters.availablePolls.length : 0;
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            await ctx.editMessageText('Loading poll list...', 
              Markup.inlineKeyboard([[Markup.button.callback('Cancel', 'cancel_check')]])
            );
          } catch (finalError) {
    ...
    if (action === 'page_prev') {
      try {
        ...
        return await handleOptionSelection(ctx, `option_${sessionData.optionIndex}`);
      } catch (error) {
    ...
    if (action === 'page_next') {
      try {
        ...
        const totalItems = Array.isArray(sessionData.selectedVoters) ? sessionData.selectedVoters.length : 0;
    ...
    // Unknown action
    await ctx.answerCbQuery();
    logger.debug('Unknown action received', { action, stage: ctx.session.checkVoters?.stage });
  };
  ...
  const initializeScene = async (ctx) => {
    ...
    if (ctx.scene.state.userPolls && Array.isArray(ctx.scene.state.userPolls)) {
      ...
      logger.debug('Initializing checkVoters from private chat with user polls', { 
        pollsCount: ctx.scene.state.userPolls.length,
        userId: ctx.from?.id
      });
      
      // Extract chatId from the first poll if available for future use
      if (ctx.scene.state.userPolls.length > 0 && ctx.scene.state.userPolls[0].chatId) {
        chatId = ctx.scene.state.userPolls[0].chatId;
      }
    }
    // Check if we have state data from previous scene with group ID
    else if (ctx.scene.state.groupId && !isNaN(parseInt(ctx.scene.state.groupId, 10))) {
      ...
      if (args.length > 0 && !ctx.isAnyGroup) {
        ...
        if (!isNaN(parsedGroupId)) {
          ...
          // Invalid group ID
          await ctx.reply(t('poll.invalidGroupId') || 'Invalid group ID. Please provide a valid group ID.');
          ctx.scene.state.silentLeave = true;
          return ctx.scene.leave();
        }
      } else if (!ctx.isAnyGroup) {
        // In private chat without valid parameters
        await ctx.reply(t('poll.privateCheckHelp') || 'To check poll in private chat, use format: /checkvoters <group_id> [message_id]');
        ctx.scene.state.silentLeave = true;
        return ctx.scene.leave();
      } else {
    ...
    if (ctx.message) {
      ...
      if (!fromPrivate) {
        try {
          ...
          logger.warn('Could not delete original /checkvoters command message:', error);
        }
      }
    }
  ...
  const showPolls = async (ctx) => {
    ...
    try {
      ...
      if (!chatId && !ctx.session.checkVoters.userPolls) {
        ...
        try {
          await ctx.reply(t('poll.invalidGroupId') || 'Invalid or missing group ID.');
          ctx.scene.state = ctx.scene.state || {};
          ctx.scene.state.silentLeave = true;
          return ctx.scene.leave();
        } catch (error) {
      ...
      if (ctx.session.checkVoters.userPolls && Array.isArray(ctx.session.checkVoters.userPolls)) {
        ...
        ctx.session.checkVoters.availablePolls = ctx.session.checkVoters.userPolls.filter(poll => 
        ...
        logger.debug('Poll search query:', {
          chatId: chatId,
          specific_messageId: ctx.session.checkVoters.specificMessageId || 'none',
        ...
        try {
          ...
          const recentPolls = await Poll.find(query)
          .sort({ createdAt: -1 });
      ...
      ctx.session.checkVoters.availablePolls = ctx.session.checkVoters.availablePolls || [];
      
      // Log found polls
      logger.debug('Found polls for display', { 
        count: ctx.session.checkVoters.availablePolls.length,
        pollIds: ctx.session.checkVoters.availablePolls.map(p => p.id || 'unknown')
      });
      ...
      const currentPagePolls = (ctx.session.checkVoters.availablePolls || []).slice(startIndex, endIndex);
      
      // Create keyboard with available polls
      try {
        const pollButtons = currentPagePolls.map(poll => {
          try {
            // Create short title (max 30 chars)
            const shortTitle = (poll.title || 'Untitled Poll').length > 30 
            ...
            return [Markup.button.callback(
              ...
              `poll_${poll.messageId}_${poll.chatId || ctx.session.checkVoters.chatId}`
            )];
          } catch (pollError) {
            ...
            return [Markup.button.callback(
              ...
              `poll_${poll.messageId || '0'}_${poll.chatId || ctx.session.checkVoters.chatId || '0'}`
            )];
          }
        ...
        if (totalPages > 1) {
          pollButtons.push([
            ...
            Markup.button.callback(
              ...
              currentPage < totalPages - 1 ? 'polls_page_next' : 'polls_page_none',
        ...
        if (ctx.callbackQuery) {
          ...
          if (ctx.session.checkVoters.fromPrivate) {
            if (ctx.session.checkVoters.userPolls) {
              ...
              messageText = `${t('scenes.voters.pollsInGroup')} ${chatId}:\n\n${t('scenes.voters.selectPoll')}`;
            }
          } else {
        ...
        try {
          ...
          const message = ctx.callbackQuery 
            ? await ctx.editMessageText(t('scenes.voters.errorLoadingPolls'), Markup.inlineKeyboard([[Markup.button.callback('Cancel', 'cancel_check')]]))
            : await ctx.reply(t('scenes.voters.errorLoadingPolls'), Markup.inlineKeyboard([[Markup.button.callback('Cancel', 'cancel_check')]]));
        } catch (finalError) {
  ...
  const handlePollSelection = async (ctx, action) => {
    ...
    const parsedChatId = parts.length > 1 ? parseInt(parts[1], 10) : ctx.session.checkVoters.chatId;
    ...
    logger.debug('Parsed poll data', {
      ...
      availablePollsCount: (ctx.session.checkVoters.availablePolls || []).length
    });
      
    // Find the selected poll from available polls for more data
    const selectedPoll = (ctx.session.checkVoters.availablePolls || []).find(
      p => p.messageId === parsedMessageId && (p.chatId === parsedChatId || !p.chatId)
    );
    ...
    try {
      logger.debug('Finding poll with messageId:', { messageId: parsedMessageId, chatId: parsedChatId });
      ...
      if (!poll) {
        logger.debug('Poll not found with primary search, trying alternate methods', {
          chatId: parsedChatId,
          messageId: parsedMessageId
        });
      ...
      if (ctx.session.checkVoters.fromPrivate) {
        optionButtons.push([Markup.button.callback(
      ...
      if (poll.options && Array.isArray(poll.options)) {
        ...
        optionButtons.push([Markup.button.callback('1. Default Option', 'option_0')]);
      }
      
      // Add back and cancel buttons
      optionButtons.push([
        Markup.button.callback('⬅️ ' + t('scenes.common.backButton'), 'back_to_polls'),
        Markup.button.callback(t('scenes.voters.cancelButton'), 'cancel_check')
      ]);
      
      // Add special note for tracked polls
      const trackedNote = poll.isTracked ? t('scenes.voters.trackedPollNote') : '';
  ...
  const handleOptionSelection = async (ctx, action) => {
    ...
    try {
      ...
      if (optionIndex < 0 || !poll.options || optionIndex >= poll.options.length) {
        logger.warn('Invalid option index', { optionIndex, optionsLength: poll.options?.length || 0 });
        await ctx.answerCbQuery(t('voters.invalidOption', { count: poll.options?.length || 0 }));
        return;
      }
      ...
      if (!option) {
        ...
        logger.warn('Option not found at index', { optionIndex, pollId: poll._id });
        ...
        logger.warn('Unexpected option format', { option, optionType: typeof option });
      ...
      if (voterIds.length === 0 && existingVotes === 0) {
        ...
        if (poll.isTracked) {
          if (poll.total_voter_count && poll.total_voter_count > 0) {
            ...
            noVotersMessage = t('voters.cannotDetermineVotes', { option: optionText, count: poll.total_voter_count }) || 
        ...
        await ctx.editMessageText(
          ...
          Markup.inlineKeyboard([
            [Markup.button.callback('⬅️ ' + t('scenes.common.backButton'), 'back_to_polls')],
            [Markup.button.callback(t('scenes.voters.finishButton'), 'finish_check')]
          ])
        );
      ...
      const { votersList, mentionsList, anonymousCount } = await processVoters(ctx, poll, voterIds || []);
      ...
      const totalExistingVotes = poll && poll.options && Array.isArray(poll.options) 
        ? poll.options.reduce((sum, opt) => sum + (opt?.existingVotes || 0), 0) 
        : 0;
      
      // Join user names with commas for display (with safety checks)
      const formattedVotersList = Array.isArray(votersList) && votersList.length > 0 
        ? votersList.join(', ') 
        : '';
        
      // Create a string of mentions for copy-paste
      const formattedMentionsList = Array.isArray(mentionsList) && mentionsList.length > 0 
        ? mentionsList.join(' ') 
        : '';
      ...
      if (hasAnonymousVotes) {
        ...
        if (poll.isTracked && existingVotes > 0 && anonymousCount > 0) {
          ...
          anonymousVotesText = t('voters.mixedAnonymousVotes', { 
          ...
          anonymousVotesText += `\n\n${t('voters.trackingLimitation') || 'Note: Due to Telegram API limitations, votes made before tracking cannot be identified. Track polls before voting starts for best results.'}`;
        } else if (poll.isTracked && existingVotes > 0) {
          // Only pre-tracking votes
          anonymousVotesText = t('voters.existingVotes', { count: existingVotes }) || 
          ...
          anonymousVotesText = t('voters.anonymousVotes', { count: anonymousCount }) || 
            `Plus ${anonymousCount} anonymous votes`;
        } else {
          // Generic fallback
          anonymousVotesText = t('voters.totalAnonymousVotes', { count: totalAnonymousVotes }) || 
      ...
      if (action.startsWith('option_') && !ctx.callbackQuery?.data?.startsWith('page_')) {
      ...
      if (fromPrivate && ctx.session?.checkVoters?.selectedPollTitle) {
        ...
        if (poll && poll.isTracked && poll.trackedAt) {
          try {
            ...
            message.push(`🔍 ${t('voters.trackedSince', { date: trackingDate }) || `Tracking since: ${trackingDate}`}\n`);
          } catch (error) {
      ...
      message.push(`${t('voters.optionVoters', { option: optionText, count: namedVotersCount })}`);
      ...
      const actionButtons = [
        [Markup.button.callback(t('scenes.voters.createPollButton'), 'create_poll_with_users')],
        ...paginationButtons,
        [Markup.button.callback('⬅️ ' + t('scenes.common.backButton'), 'back_to_polls')],
        [Markup.button.callback(t('scenes.voters.finishButton'), 'finish_check')]
      ];
      ...
      try {
        ...
        // Если есть поименованные избиратели на текущей странице, отображаем их
        if (currentPageVoters.length > 0) {
          fullMessage += `\n\n${t('voters.list') || 'Voters:'}`;
          
          // Добавляем каждого избирателя с текущей страницы в список
          currentPageVoters.forEach((voter, index) => {
            fullMessage += `\n${startIndex + index + 1}. ${voter}`;
          });
          
          // Добавляем информацию о пагинации, если у нас более одной страницы
          if (totalVoters > itemsPerPage) {
            fullMessage += `\n\n${t('voters.page') || 'Page'} ${currentPage + 1}/${Math.ceil(totalVoters / itemsPerPage)}`;
          }
        }
        ...
        if (fullMessage.length > 4000) {
          ...
          let simplifiedMessage = `${t('voters.optionVoters', { option: optionText, count: namedVotersCount })}\n`;
          
          // Add paginated voters in simplified format
          if (currentPageVoters.length > 0) {
            simplifiedMessage += `\n${t('voters.list') || 'Voters:'}`;
            currentPageVoters.forEach((voter, index) => {
              simplifiedMessage += `\n${startIndex + index + 1}. ${voter}`;
          ...
          await ctx.editMessageText(simplifiedMessage, Markup.inlineKeyboard(actionButtons));
        } else {
        ...
        if (fromPrivate && ctx.session.checkVoters.selectedPollTitle) {
          ...
          fallbackMessage += `👥 ${t('scenes.voters.pollInGroup')} ${chatId}\n\n`;
        }
        
        // Calculate named voters count - excluding anonymous
        const namedVotersCount = voterIds.length;
        
        fallbackMessage += `${t('voters.optionVoters', { option: optionText, count: namedVotersCount })}\n`;
        ...
        if (poll.isTracked && poll.trackedAt) {
          const trackingDate = new Date(poll.trackedAt).toLocaleString();
          fallbackMessage += `\n\n🔍 ${t('voters.trackedSince', { date: trackingDate }) || `Tracking since: ${trackingDate}`}\n`;
        }
      ...
      try {
        ...
        if (!mentionedUser && voterId < 0) {
          try {
            ...
            const chatMember = await ctx.telegram.getChatMember(chatId, Math.abs(voterId))
              .catch(() => null);
            
            if (chatMember && chatMember.user) {
              mentionedUser = poll.mentions.find(m => 
                m && m.username && chatMember.user.username && 
                m.username.toLowerCase() === chatMember.user.username.toLowerCase()
              );
            }
          } catch (error) {
            logger.warn(`Error fetching chat member for user ID ${Math.abs(voterId)}:`, error);
          }
        }
        ...
        if (mentionedUser && mentionedUser.username) {
          ...
          try {
            const chatMember = await ctx.telegram.getChatMember(chatId, mentionedUser.userId);
            if (chatMember && chatMember.user) {
            ...
            logger.warn(`Could not get chat member info for mentioned user ${mentionedUser.userId}:`, error);
            votersList.push(`User ${mentionedUser.userId}`);
            mentionsList.push(`@${mentionedUser.userId}`);
          }
        } else {
          ...
          try {
            ...
            logger.warn(`Could not get chat member info for user ${voterId}:`, error);
            votersList.push(`User ${voterId}`);
            mentionsList.push(`@${voterId}`);
          }
        }
  ...
  const handleCreatePoll = async (ctx) => {
    ...
    const { selectedVoters, mentionsList, selectedOption } = ctx.session.checkVoters;
    ...
    const suggestedTitle = `${t('scenes.voters.followUpTitle', { option: selectedOption })}`;
    ...
    try {
      ...
      logger.warn('Could not update message when starting poll creation:', error);
    }
    ...
    const checkData = {
      ...
      originalPollTitle: ctx.session.checkVoters.selectedPollTitle || ctx.session.checkVoters.pollTitle || 'Untitled Poll' // Original poll title
    };
